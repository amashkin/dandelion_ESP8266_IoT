...
void statusHandle() {
  ...
  sStatus = "Fan: " + sToNextSwitch01 + " WPump: " + sToNextSwitch02 + " Hum: " + iHumidity + " Tem: " + iTemperature;
}

void danLogicHandle() {
  htSensorHandle();  // HT Sensor 
  switch01Handle();  // Fan switch handle. Job to run Fan every 30 sec
  switch02Handle();  // Water pump switch handle
  statusHandle();    // Put data to string for IoT
}

void danLogicSwitch01Changed(bool bSwitch) {
  if (bSwitch) {
    Serial.println("Fan. Switch 01 On");
    digitalWrite(PIN_D5, HIGH);
  } else {
    Serial.println("Fan. Switch 01 Off");
    digitalWrite(PIN_D5, LOW);
  }
}

void danLogicSwitch02Changed(bool bSwitch) {
  if(bSwitch) 
